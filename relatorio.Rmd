...
```{r}
Ano <- aggregate(x = list(QT = respostas$Ano), by = list(Tratamento = respostas$Ano, Bloco = respostas$Fara_Computacao), FUN=length)
knit_print(Ano)

fit <- anova(lm(QT ~ Tratamento + Bloco, data = Ano))
cat("\nTratamento: Ano\n")
print(fit)
```
...
for (i in 2:ncol(amostra_anova)) {
  i = 3
  temp <- aggregate(list(QT = amostra_anova[, i]), by = list(Tratamento = amostra_anova[, i], Bloco = amostra_anova$Fara_Computacao), FUN=length)

  # complementar temp com dominios zerados
  temp2 <- expand.grid(Bloco = levels(amostra_anova$Fara_Computacao), Tratamento = levels(amostra_anova[, i]))
  ...
  fit <- anova(lm(QT ~ Tratamento + Bloco, data = temp))
  print(fit)
